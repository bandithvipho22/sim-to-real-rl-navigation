...
class RobotController(Node):
    def __init__(self):
        ...
        self.max_angular_velocity = 0.35

        # Initial Blend factor for RL and Pure Pursuit (0: full RL, 1: full Pure Pursuit)
        self.pp_blend_factor = 0.6

        # Parameters for exploration phases
        self.exploration_phase = True  # Start with RL exploration phase
        self.exploration_timer = self.create_timer(10.0, self.toggle_exploration_phase)  # Switch phases every 10 seconds
        self.training_steps = 0  # To gradually reduce controller's influence

    def toggle_exploration_phase(self):
        """Switch between RL exploration and Pure Pursuit controlled phases."""
        self.exploration_phase = not self.exploration_phase
        phase = "Exploration (RL)" if self.exploration_phase else "Controlled (Pure Pursuit)"
        self.get_logger().info(f"Switched to: {phase}")

    def adjust_blend_factor(self):
        """Gradually reduce the influence of Pure Pursuit over time."""
        self.training_steps += 1
        if self.training_steps % 1000 == 0:  # Reduce every 1000 steps
            self.pp_blend_factor = max(0.0, self.pp_blend_factor - 0.05)  # Decrease by 0.05 until 0.0 (full RL)
            self.get_logger().info(f"Blend factor adjusted: {self.pp_blend_factor}")
    ...
    def blend_actions(self, rl_action, pp_action):
        linear_vel = (1 - self.pp_blend_factor) * rl_action[0] + self.pp_blend_factor * pp_action[0]
        angular_vel = (1 - self.pp_blend_factor) * rl_action[1] + self.pp_blend_factor * pp_action[1]
        return [linear_vel, angular_vel]

    def execute_action(self, rl_action):
        pp_action = self.pure_pursuit_control()

        # If in exploration phase, give full control to RL agent, else blend with Pure Pursuit
        if self.exploration_phase:
            blended_action = rl_action  # Full RL control
        else:
            blended_action = self.blend_actions(rl_action, pp_action)  # Blended control

        self.send_velocity_command(blended_action)

        # Adjust blend factor over time for gradual RL autonomy
        self.adjust_blend_factor()

        # Publish action type
        action_type_msg = String()
        action_type_msg.data = f"Blended (RL: {1-self.pp_blend_factor:.2f}, PP: {self.pp_blend_factor:.2f})"
